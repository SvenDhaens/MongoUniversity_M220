...
public class UsingAggregationBuilders extends AbstractLesson {
   ...
   */
    ...
    */
    ...
    @Test
    public void testGroupCastSet() {
        List<Bson> pipeline = new ArrayList<>();

        Bson unwind = Aggregates.unwind("$cast");
        Bson groupCastSet = Aggregates.group("", Accumulators.addToSet("castlist", "$cast"));

        pipeline.add(unwind);
        pipeline.add(groupCastSet);

        int countDocs = 0;
        AggregateIterable<Document> aggregate = this.moviesCollection.aggregate(pipeline);
